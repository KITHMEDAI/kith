...
export async function POST(req: NextRequest) {
  ...
  const { data: session } = await supabase
    .from('sessions')
    .select('id, session_number, started_at, ended_at, soap_note, key_points, session_summary, homework_assigned, session_growth, prescription_notes, patient:patients(display_name, diagnosis, date_of_birth)')
    .eq('id', sessionId)
...
<body>
  ...
  ${s.session_growth && (s.session_growth.areas_of_progress?.length || s.session_growth.areas_of_concern?.length || s.session_growth.narrative) ? `
  <div class="section">
    <div class="section-title">Progress &amp; Trajectory</div>
    ${s.session_growth.narrative ? `<div class="section-body">${boldify(s.session_growth.narrative)}</div>` : ''}
    ${s.session_growth.areas_of_progress?.length ? `<div class="meta-label" style="margin-top:8px">Progress</div><ul class="key-points">${s.session_growth.areas_of_progress.map(p => `<li>${boldify(p)}</li>`).join('')}</ul>` : ''}
    ${s.session_growth.areas_of_concern?.length ? `<div class="meta-label" style="margin-top:8px">Concerns</div><ul class="key-points">${s.session_growth.areas_of_concern.map(p => `<li>${boldify(p)}</li>`).join('')}</ul>` : ''}
  </div>` : ''}

  ${s.prescription_notes && (s.prescription_notes.medication_relevant || s.prescription_notes.note) ? `
  <div class="section">
    <div class="section-title">Medication Notes</div>
    <div class="section-body">
      ${s.prescription_notes.note ? boldify(s.prescription_notes.note) : 'Medication-relevant content flagged this session.'}
      ${s.prescription_notes.refer_to_psychiatrist ? '<br/><strong>Psychiatrist referral suggested.</strong>' : ''}
    </div>
  </div>` : ''}
